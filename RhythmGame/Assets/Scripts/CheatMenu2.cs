...
public class CheatMenu2 : MonoBehaviour {
    ...
    void Update() {
        ...
        //toggle cheat menu
        if (Input.GetKeyDown(KeyCode.F2) && GM.timecount <= 86 || Input.GetKeyDown(KeyCode.Alpha2) && GM.timecount <= 86) {
            if (!GamePaused) {
    ...
    public void perfectHits(bool value) {
        if (value == true) {
            NoteObject2.cheat = true;

        }
        else {
            NoteObject2.cheat = false;
